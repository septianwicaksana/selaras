...
function EditCustomer() {
  ...
  const customerUpdateStatus = useSelector((state) => state.customers.customerUpdateStatus)

  const canSave = customerUpdateStatus === 'idle'
  ...
  useEffect(() => {
    ...
    reset({
      name: customerById.name, //null
      ktp: customerById.ktp, //not-null
      phone: customerById.phone,
      address: customerById.address,
      location: customerById.location,
      pos: customerById.pos,
      pob: customerById.pob,
      dob: customerById.dob,
      job: customerById.job,
      rent: customerById.rent,
    })
  }, [id, customerById, customerByIdStatus, reset, dispatch])
  ...
  const onSubmit = async (data) => {
    if (canSave)
      try {
        ...
        dispatch(clearCustomerUpdateStatus())
        dispatch(clearCustomerListStatus())
        navigate('/branches')
      }
  }
